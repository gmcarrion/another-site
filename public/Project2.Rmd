...
```
This is a data frame with 1295 rows and 10 columns.  
The data set contains information about the mortality outcomes for females suffering myocardial infarction.
A myocradial infarction is a heart attack and I will refer to it as that throughout the project.
It contains a bindary variable, 'outcome' in which the patient either lived or died.
The data set contains 2 numerical variable,'age' the age of the onset of the myocardial infarction, and 'yronset' the year of the onset of the myocardial infarction.
The data set contains 6 categorical variables 'premi' previous myocardial infarctions, 'smstat' smoking status, 'diabetes' diabetes, 'highbp' high blood pressure, 'hichol' high cholesterol, 'angina' angina, 'stroke' if they previouly has a stroke.
Each categorical variable has levels 'y', yes, 'n', no, 'nk', not known.
...
mean(data$age) #60.77988
```

```{R}
fit <- lm(age_c ~ premi * diabetes, data=data)
summary(fit)
ggplot(data, aes(y=age, x=premi,group=diabetes))+
...
```
The robust standard errors and the bootstrap standard errors were very similar (within 1% of each other).
The p-values of the diabetes and the interaction of the two models were very similar, while the p-values of the intercept and the premi variable were much lower than on the bootstrapped data than on the robust model.
